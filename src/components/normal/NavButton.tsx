        ...
        >
          ...
          >
            {menuItems.map((item, index) => (
              <div 
                key={item.label} 
                className={`w-full text-center shrink-0 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${isMenuOpen ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}
                style={{ transitionDelay: isMenuOpen ? `${0.2 + index * 0.07}s` : '0s' }}
              >
                <button
                  onClick={() => {
                    if (item.action) {
            ...
            <div 
              className={`mt-8 shrink-0 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${isMenuOpen ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}
              style={{ transitionDelay: isMenuOpen ? `${0.2 + menuItems.length * 0.07}s` : '0s' }}
            >
              <Link
